...
pub fn tombstone_to_dict(t: &Tombstone) -> DaemonResult<Dict> {
    let mut d = Dict::new();
    insert_str(&mut d, "name", &t.name)?;
    insert_str(&mut d, "base_subvol", &t.base_subvol)?;
    insert_str(&mut d, "id", t.id.as_str())?;
